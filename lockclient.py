...
# Module-level lock registry — keyed by id(rp_obj).
# Storing threading.Lock directly on RP_client objects makes them unpicklable,
# which breaks mp.Process on Windows (spawn context).  Using an external dict
# keeps the RP_client objects clean and picklable.
_lsock_locks = {}

def _get_lsock_lock(rp_obj):
    k = id(rp_obj)
    if k not in _lsock_locks:
        import threading as _thr
        _lsock_locks[k] = _thr.Lock()
    return _lsock_locks[k]
...
class LockClient(Sender):
    ...
    def _raw_loop_send(self, RP, action, value=""):
        """
        Send a command directly to the running reaction_loop on port 5065
        using raw socket I/O, completely bypassing the shared Sender.sel.

        This avoids the 3-thread race condition on the Windows SelectSelector
        that occurs when the start_loop background thread, the Sender event_loop
        thread, and the calling cell all call sel.select() simultaneously.

        Based on the same pattern as Monitor._raw_send / Monitor._port5066_send.
        """
        import json as _j, struct as _st, selectors as _sel, io as _io
        ...
        lsock  = rp_obj.lsock
        if lsock is None:
            print(f"_raw_loop_send: {RP} has no loop socket — is a loop running?")
            return None

        content = _j.dumps({"action": action, "value": value},
                            ensure_ascii=False).encode("utf-8")
        jh_bytes = _j.dumps({
            "byteorder": "little", "content-type": "text/json",
            "content-encoding": "utf-8", "content-length": len(content),
        }, ensure_ascii=False).encode("utf-8")
        raw = _st.pack(">H", len(jh_bytes)) + jh_bytes + content

        with _get_lsock_lock(rp_obj):
            ...
            while _time() - t0 < 5:
                ...
                for key, mask in events:
                    if mask & _sel.EVENT_WRITE and not sent:
                        ...
                        try:
                            ...
                            # the lock loop socket was closed (lock stopped or
                            # board disconnected). Return None gracefully.
                            print(f"_raw_loop_send: socket error during recv: {_e}")
                ...
                if expected_len is None and len(buf) >= 2:
                    jh_len = _st.unpack(">H", buf[:2])[0]
                    if len(buf) >= 2 + jh_len:
                        jh = _j.loads(buf[2:2 + jh_len].decode("utf-8"))
        ...
        response = _j.loads(
            _io.TextIOWrapper(_io.BytesIO(data),
                              encoding=jh["content-encoding"], newline="").read()
        )
        return response.get("result")

    def stop_loop(self, RP):
        """
        Stop the loop running on RP_Scan (scan or lock).

        For scan loops: sends "stop_scan" first (disables Out2) then "stop".
        For lock loops: sends "stop" only — "stop_scan" is not in the lock
          reaction_loop and sending it would corrupt the shared lsock state.

        Both sends use _raw_loop_send (raw socket I/O on lsock) instead of
        going through Sender.sel.  During a running loop the start_loop
        background thread continuously calls sel.select() in its while-True
        wait loop; adding a new registration from a second thread causes
        Windows SelectSelector to deadlock or raise a KeyError.
        """
        if not self.RPs[RP].loop_running:
            print(f"{RP}: no loop is running.")
            return

        active = getattr(self.RPs[RP], "_loop_action", None)
        if self.RPs[RP].mode in ("scan", "scan_mon") and active == "start_scan":
            # Disable Out2 immediately before sending the loop-stop command.
            try:
                self._raw_loop_send(RP, "stop_scan", "")
            except Exception as e:
                print(f"stop_loop: stop_scan failed: {e}")

        result = self._raw_loop_send(RP, "stop", "")
        # Mark loop as stopped on the PC side.  The start_loop background thread
        # will also see the port-5000 "Done!" response arrive and will set
        # lsock = None itself, but setting loop_running = False here ensures
        # subsequent _check_for_loop guards pass immediately.
        self.RPs[RP].loop_running = False
        return result

    def shift_lockpoint(self, RP, new_lockpoint_ms):
        """
        Move the cavity lockpoint while the PID lock is running.

        Sends the new lockpoint to the board's lock reaction_loop on port 5065.
        The board shifts the PID integrator by the same delta so the transition
        is bumpless — the peak glides smoothly to the new position rather than
        being kicked.

        Use this to:
          • Deliberately move the locked peak to confirm the PID is following
          • Find a better operating point without stopping the lock
          • Verify lock bandwidth (slow vs fast lockpoint steps)
        """
        if not self.RPs[RP].loop_running:
            print(f"{RP}: lock not running — start_lock first.")
            return
        # Update PC-side settings so the monitor markers move immediately.
        self.RPs[RP].settings["Master"]["lockpoint"] = float(new_lockpoint_ms)
        self.set_monitor(RP)
        return self._raw_loop_send(RP, "shift_lockpoint", float(new_lockpoint_ms))

    def set_lock_amplitude(self, RP, amplitude):
        """
        Change the ramp amplitude while the cavity lock is running.

        The ramp amplitude is the half-swing of the OUT2 triangle wave.
        Changing it while locked widens or narrows the scan window around
        the locked peak — the PID is unaffected (it only controls the DC offset).

        Typical use: reduce amplitude to tighten the scan range around the
        locked peak and confirm the peak stays centred.
        """
        if not self.RPs[RP].loop_running:
            print(f"{RP}: lock not running — start_lock first.")
            return
        return self._raw_loop_send(RP, "set_amplitude", float(amplitude))
    ...
    def start_loop(self, RP, action, value="Hello world!"):
        ...
        """
        self.RPs[RP]._loop_action = action   # track so stop_loop knows what to stop
        t = threading.Thread(
    ...
    def start_lock(self, RP):
        ...
        # stop_loop now uses _raw_loop_send (bypasses selector) so this is safe
        # even though loop_running is True.
        if self.RPs[RP].loop_running and self.RPs[RP].mode in ("scan", "scan_mon"):
            self.stop_loop(RP)
            sleep(1.5)   # board finishes scan cleanup before lock opens port 5065
...
class Monitor(Sender):
    ...
    def _raw_send(self, action, value="0"):
        ...
        # _get_lsock_lock() returns a threading.Lock from the module-level
        # registry (not stored on the RP object, so RP remains picklable).
        lsock = self.RP.lsock
                ...
                for key, mask in events:
                    if mask & _sel.EVENT_WRITE and not sent:
                        ...
                        try:
                            ...
                            print(f"Monitor._raw_send: socket error during recv: {_e}")
                            local_sel.unregister(lsock)
                            local_sel.close()
                            return None
