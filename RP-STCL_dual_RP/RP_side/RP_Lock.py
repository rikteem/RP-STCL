...
class RP_Server(Receiver):  # handles socket communication from redpitaya side
    ...
    def action_start_scan(self, query):
        ...
        def iteration():
            ...
            rl.var_dict["i"] += 1
            # AWG keepalive: on OS 2.x the generator can silently stop outputting
            # after thousands of ADC re-arm cycles due to FPGA AWG state drift.
            # Re-asserting rp_GenOutEnable every 500 cycles (~2 s at dec=32)
            # is safe (it is NOT one of the calls that reset the AWG state machine)
            # and reliably prevents the ramp from dying mid-scan.
            if rl.var_dict["i"] % 500 == 0:
                _rp.rp_GenOutEnable(_CH[0])   # Out1 — trigger square wave
                _rp.rp_GenOutEnable(_CH[1])   # Out2 — scan ramp
