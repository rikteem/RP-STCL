...
def SG_deriv(x,y,r, m = m_1):
    ...
    denom = dv[1] - dv[0]
    if denom == 0.0:
        # Flat derivative — no sign change near peak (flat top or no peak in
        # range).  Fall back to argmax position to avoid divide-by-zero.
        return np.array([x[j], y[j]])
    x_p = x[j] - dv[0] * (x[j+1]-x[j]) / denom
