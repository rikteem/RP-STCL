...
class reaction_loop(Receiver):  #### USE THIS FOR THE LOCKING LOOP
    ...
    def start_loop(self):
        ...
        try:
            ...
            # Always close the server socket when the loop exits or crashes.
            # Without this, if setup() raises an exception the OS keeps port
            # 5065 bound; the next reaction_loop.start_loop() call then fails
            # with OSError: [Errno 98] Address already in use.
            try:
                self.sock.close()
            except Exception:
                pass
...
class RP_Server(Receiver):  # handles socket communication from redpitaya side
    ...
    def action_start_lock(self, query):
        if self.RP_mode in ["scan", "lock"]:
            ...
            if self.RP_mode == "scan":
                # Re-enable OUT2 ramp — scan_output_disable() was called when the
                # scan loop exited.  The lock PID adjusts gen_ramp.offset each cycle
                # to shift the peaks; OUT2 must be active or the piezo never moves.
                self.lock.scan_output_enable()

            # ── Live-control actions for the lock reaction_loop (port 5065) ──────
            # These are added BEFORE self.lock.start() so they are available in
            # the lock's action_dict when setup_server(loop=True) is called.

            def rl_shift_lockpoint(new_lp):
                """Bumpless lockpoint shift: compensates PID integrator for the delta."""
                new_lp = float(new_lp)
                if "Master" not in self.lock.settings:
                    return "Master not in settings"
                s   = self.lock.settings["Master"]
                old = s.get("lockpoint", new_lp)
                delta = new_lp - old
                s["lockpoint"] = new_lp
                self.lock.Master_pos = new_lp
                pid = s.get("PID")
                if pid is not None and self.lock.FSR_ref and self.lock.FSR_ref > 0:
                    # Shift the integrator state so PID.MV absorbs the step
                    # immediately — the next step() call sees ~zero error.
                    pid.I_val += delta / self.lock.FSR_ref
                    pid.MV     = pid.I_val  # approximate; recalculated in update()
                return "lockpoint shifted to {:.4f} ms".format(new_lp)

            def rl_set_amplitude(amp):
                """Change OUT2 triangle amplitude without touching the PID offset."""
                amp = float(amp)
                rp.rp_GenAmp(_CH[1], amp)
                self.lock._scan_amplitude = amp
                return "amplitude set to {:.3f} V".format(amp)

            def rl_get_pid_state(query):
                """Return PID MV, I_val, and limits for each locked channel."""
                state = {}
                for key, val in self.lock.settings.items():
                    if "PID" in val:
                        pid = val["PID"]
                        state[key] = {
                            "MV":    pid.MV,
            ...
            self.lock.action_dict["shift_lockpoint"] = rl_shift_lockpoint
            self.lock.action_dict["set_amplitude"]   = rl_set_amplitude
            self.lock.action_dict["get_pid_state"]   = rl_get_pid_state

            # Keepalive: re-assert rp_GenOutEnable every 500 lock iterations
            # (~1 s at dec=16) to prevent the DAC output from being silently
            # disabled mid-lock.  Do NOT call rp_GenTriggerOnly — repeated
            # calls in CONTINUOUS mode corrupt the generator state on OS 2.x.
            _orig_lock_iter = self.lock.iteration
            _kp_count = [0]
            def _lock_iter_keepalive(*a, **kw):
                _orig_lock_iter(*a, **kw)
                _kp_count[0] += 1
                if _kp_count[0] % 500 == 0:
                    rp.rp_GenOutEnable(_CH[0])
                    rp.rp_GenOutEnable(_CH[1])
            self.lock.iteration = _lock_iter_keepalive
    ...
    def action_start_scan(self, query):
        ...
        def iteration():
            ...
            # exception is logged but does NOT propagate out of iteration().
            # Without this guard, an unhandled exception escapes start_server's
            # inner loop, hits the outer try/except, exits start_server, and
            # triggers scan_output_disable() cleanup — giving a silent flat-0 V
            # ramp with port 5065 still open (making the loop look alive from
            # the PC side while the board has actually stopped scanning).
            try:
                self.lock.trigger()
                import rp as _rp
                ...
                rl.var_dict["i"] += 1
                # AWG keepalive: re-assert rp_GenOutEnable every 500 cycles (~1 s
                # at dec=16) to keep the DAC output path enabled.
                # Do NOT call rp_GenTriggerOnly — repeated calls in CONTINUOUS
                # mode corrupt the generator state on OS 2.x (amplitude → 0).
                if rl.var_dict["i"] % 500 == 0:
                    _rp.rp_GenOutEnable(_CH[0])
                    _rp.rp_GenOutEnable(_CH[1])
            except Exception as _iter_exc:
                import traceback as _tb
                print("iteration() exception (scan loop continues): {}".format(_iter_exc))
                _tb.print_exc()
    ...
    def action_monitor(self, query):
        ...
        # Shared cache for port 5066 monitor server (same pattern as action_start_scan)
        import threading as _thr
        _cache66_lock = _thr.Lock()
        _cache66 = {0: None, 1: None}
        _mon66_stop = _thr.Event()

        def _monitor_server_66():
            """Dedicated monitor server on port 5066 for monitor-mode boards."""
            import socket as _sock, json as _json, struct as _struct
            srv = _sock.socket(_sock.AF_INET, _sock.SOCK_STREAM)
            srv.setsockopt(_sock.SOL_SOCKET, _sock.SO_REUSEADDR, 1)
            srv.bind((self.addr[0], 5066))
        ...
        rl.iteration = iteration
        _mon66_stop.clear()
        _mon66_srv_thread = _thr.Thread(target=_monitor_server_66, daemon=True)
        _mon66_srv_thread.start()
        rl.start_loop()
        _mon66_stop.set()   # stop port 5066 server cleanly when loop exits
        t = perf_counter() - t0
        i = rl.var_dict["i"]
...
class RP:
    ...
    def __init__(self, mode="scan"):
        ...
        # Trigger source — ALL modes use RP_TRIG_SRC_CHB_PE (In2 positive edge).
        #
        # Physical wiring:
        #   RP_Scan Out1 (square wave) is connected via BNC cable to In2 of
        #   EVERY board (RP_Scan itself, RP_Mon, RP_Lock).  The ADC fires on
        #   the rising edge of that cable signal.
        #
        #   RP_TRIG_SRC_AWG_PE is an INTERNAL path that fires the ADC from the
        #   AWG output without a physical cable.  It was used in early testing
        #   but the actual hardware uses the external cable → In2 path, so
        #   AWG_PE never fires and every trigger() call timed out.
    ...
    def _trig_src(self):
        """
        ADC trigger source — always RP_TRIG_SRC_CHB_PE (In2 positive edge).

        All boards (scan, lock, monitor) receive the RP_Scan Out1 square wave
        on their In2 via a physical BNC cable and trigger from that edge.
        """
    ...
    def scan_output_enable(self, waveform=None, amplitude=None, offset=None):
        ...
        # OS 2.x AWG call discipline
        # ─────────────────────────────────────────────────────────────────────
        # rp_GenWaveform / rp_GenMode / rp_GenTriggerSource / rp_GenOutEnable
        # all reset the shared AWG state machine for BOTH channels (Ch1 + Ch2
        # share one AWG clock domain).  Calling any of them while Out1 is live
        # stops the square wave and breaks RP_TRIG_SRC_AWG_PE, causing trigger()
        # to time out on every subsequent call.
        #
        # Strategy:
        #   • rp_GenWaveform/Mode/TriggerSource/OutEnable — first call ONLY.
        #     After that, _scan_waveform is cached so these are never repeated,
        #     even across scan_output_disable / re-enable cycles.
        #   • rp_GenFreqDirect / rp_GenAmp / rp_GenOffset — safe every call.
        #   • After the first-call AWG-disrupting block: explicitly restart Out1
        #     (rp_GenTriggerOnly _CH[0]) and re-arm the ADC trigger coupling
        #     (rp_AcqStart + rp_AcqSetTriggerSrc) so trigger() works immediately.
        #   • scan_output_disable() zeros amplitude instead of calling
        #     rp_GenOutDisable — that way the re-enable path only needs
        #     rp_GenAmp/Offset, never rp_GenOutEnable, and Out1 is never
        #     disturbed on the scan→lock transition.
        ...
        if _first_call:
            ...
            rp.rp_GenTriggerSource(ch, rp.RP_GEN_TRIG_SRC_INTERNAL)
            rp.rp_GenOutEnable(ch)
            # rp_GenWaveform (and Mode/TriggerSource) reset the shared AWG
            # state machine — Out1 may stop running.  rp_GenTriggerOnly on
            # OS 2.x restarts a CONTINUOUS generator (per-channel, confirmed
            # working).  Re-arm the ADC after so it catches the next In2 edge.
            rp.rp_GenTriggerOnly(_CH[0])
            rp.rp_AcqStart()
            rp.rp_AcqSetTriggerSrc(self._trig_src)

        # Always-safe calls — update amplitude/offset/freq without side effects.
    ...
    def scan_output_disable(self):
        """Silence Out2 by zeroing amplitude and offset — does NOT call rp_GenOutDisable.

        Keeping the output *enabled* (but silent) means the next scan_output_enable()
        call only needs rp_GenAmp/rp_GenOffset to restore the ramp — no
        rp_GenOutEnable, which would reset the shared AWG state machine and stop
        Out1 (the trigger square wave), causing trigger() timeouts.
        """
        import traceback as _tb
        # Print a compact call stack so the log shows exactly what triggered
        # scan_output_disable — critical for diagnosing unexpected mid-scan
        # shutdowns (the "ramp dies after 2 min" problem).
        _stack_lines = _tb.format_stack()
        # Keep only the last 4 frames (skip the very bottom stdlib frames)
        _relevant = "".join(_stack_lines[-4:]).strip()
        print("scan_output_disable: Out2 SILENT (amp=0)  caller:\n" + _relevant)
        rp.rp_GenOffset(_CH[1], 0.0)
        rp.rp_GenAmp(_CH[1], 0.0)   # silence without DisableOut — no AWG reset
        # Do NOT call rp_GenOutDisable — it would require rp_GenOutEnable on
        # re-enable, which resets the shared AWG state machine and stops Out1.
        # Do NOT delete _scan_waveform — keeping it means the next enable call
        # sees _first_call=False and skips rp_GenWaveform/rp_GenMode/
        # rp_GenTriggerSource, which would reset the AWG and glitch Out1.
        self._scan_output_disabled = True
...
class RP_Lock(RP, reaction_loop):
    def __init__(self, addr, mode="lock"):
        ...
        self.mode = mode
        self.FSR_ref = None
        # Reset and trigger both generators.
        # rp_GenResetChannelSM resets the phase counter; rp_GenTriggerOnly
        # (re)starts the CONTINUOUS generator.  Both calls are per-channel on
        # OS 2.x and do NOT affect the other channel's running state.
        # gen_trig (Out1 square wave) must be started here so the physical
        # signal reaches IN2 of all boards; without this Out1 is silent.
        self.gen_trig.reset()
    ...
    def setup_lock(self):
        ...
        # Preserve ramp offset in scan mode — same reason as in start().
        # Only zero the output for laser-lock mode where outputs are PID-driven
        ...
        self.check_sign()
        # check_sign() runs up to 100 step() calls; any skipped step resets
        # self.data to an empty dict before the exception is caught by
        # update_pos(). If the last step was skipped, self.data is empty and
        # the diagnostics below raise KeyError: 'Master'.
        # Re-acquire here to guarantee fresh, valid data is available.
        for _retry in range(20):
            ...
            try:
                ...
                print("setup_lock: post-check_sign data refresh attempt {} failed: {}".format(
                    _retry + 1, _e))
        if "Master" not in self.data:
            raise RuntimeError(
                "setup_lock: could not get valid Master peak data after check_sign() — "
                "check peak ranges and trigger signal")
        print("Master_pos:", self.Master_pos)
        print("FSR_ref: {:.4f} ms".format(self.FSR_ref))
        for key, val in self.settings.items():
            if key == "Master":
                p0 = self.data[key][:, 0][0]
                p1 = self.data[key][:, 1][0]
                h0 = self.data[key][:, 0][1]
                h1 = self.data[key][:, 1][1]
                print("SETUP {:6s} | peak0={:.4f} ms (h={:.3f})  peak1={:.4f} ms (h={:.3f})  "
                      "lockpoint={:.4f} ms  FSR={:.4f} ms".format(
                      key, p0, h0, p1, h1, val["lockpoint"], self.FSR_ref))
            else:
                print("SETUP {:6s} | peak={:.4f} ms (h={:.3f})  lockpoint={:.4f} ms".format(
                      key, self.data[key][0], self.data[key][1], val["lockpoint"]))

    def init_FSR_ref(self, averages=20):
        FSRs = []
        attempts = 0
        max_attempts = averages * 3   # allow up to 3× retries for bad acquisitions
        while len(FSRs) < averages and attempts < max_attempts:
            attempts += 1
            try:
                self.update_data()
                FSRs.append(self.FSR)
            except Exception as _e:
                # Stale/zero acquisition (e.g. trigger timeout) produces an
                # empty slice in the peak finder.  Skip and retry.
                print("init_FSR_ref: bad acquisition ({}), retrying...".format(_e))
        if not FSRs:
            raise RuntimeError("init_FSR_ref: could not get a valid acquisition "
                               "after {} attempts — check trigger and peak ranges".format(max_attempts))
        self.FSR_ref = np.mean(FSRs)
        print("init_FSR_ref: got {}/{} good acquisitions, FSR={:.4f}±{:.4f} ms".format(
            len(FSRs), attempts, self.FSR_ref, np.std(FSRs)))
    ...
    def loop_iter(self, *args, **kwargs):
        ...
        if self.iter_num % 50 == 0:
            for key, val in self.settings.items():
                pos      = val.get("position", float("nan"))
                lockpt   = val.get("lockpoint", float("nan"))
                err      = self.errs.get(key, float("nan"))
                pid      = val.get("PID")
                mv       = pid.MV    if pid else float("nan")
                i_val    = pid.I_val if pid else float("nan")
                offset   = self.gen_ramp.offset if key == "Master" else float("nan")
                feedback = self.feedback
                trig_ok  = not getattr(self, "_last_trig_timeout", False)
                print(
                    "LOCK iter={:5d} | {:6s} pos={:.4f} lp={:.4f} err={:+.4f} "
                    "MV={:+.4f} I={:+.4f} offset={:+.4f} feedback={} trig_ok={}".format(
                        self.iter_num, key, pos, lockpt, err, mv, i_val,
                        offset, feedback, trig_ok
                    )
                )

    def start(self, *args, **kwargs):
        self.feedback = True
        # For laser-lock mode, reset the PID output to neutral before starting.
        # For scan mode, preserve the current ramp offset — it was set by
        # scan_output_enable() to the user's CAV_OFFSET so the peaks remain
        # visible inside CAV_RANGE.  Zeroing it would shift the peaks out of
        # the windows and prevent init_FSR_ref() from finding them.
        if self.mode != "scan":
